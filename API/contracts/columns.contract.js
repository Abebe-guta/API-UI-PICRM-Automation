...
export function validateColumnsResponse(data) {
  ...
  //   { data: [...] }
  //   [...]
  const raw = Array.isArray(data) ? data : (data?.columns ?? data?.data ?? []);
  ...
  // ----------------------------------------------------------
  // Basic structural validation of each column object
  // ----------------------------------------------------------
  raw.forEach((column, index) => {
    if (!column || typeof column !== "object") {
      throw new Error(`❌ Column at index ${index} is not an object`);
    }

    const name = column.name || column.column_name || column.field_name;

    if (!name) {
      throw new Error(`❌ Column at index ${index} missing name`);
    }
  });
  ...
  if (normalized.length === 0) {
    throw new Error("❌ No usable columns after schema normalization");
  }
