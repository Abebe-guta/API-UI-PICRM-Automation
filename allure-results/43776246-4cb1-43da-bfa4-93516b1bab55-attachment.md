...
```
Error: Pattern status must be present when in segment
    ...
    - generic [ref=e91]:
      ...
      - generic [ref=e96]:
        ...
        - generic [ref=e101]:
          - generic [ref=e102]:
            ...
            - spinbutton "Segment ID" [ref=e105]: "720"
          - generic [ref=e106]:
      ...
      - generic [ref=e113]:
        ...
        - generic [ref=e121]:
          ...
          - generic [ref=e124]:
            - text: Segment ID
            - paragraph [ref=e125]: "720"
          - generic [ref=e126]:
            - text: Segment Name
            - paragraph [ref=e127]: Seg_loan_table_coop_123409
          - generic [ref=e128]:
            - text: Segment Row ID
            - paragraph [ref=e129]: "284703"
          - generic [ref=e130]:
            - text: Pattern Status
...
```ts
  ...
  81  |     expect(resultById.data.customerId, "Customer ID must match input").toBe(
  82  |       CUSTOMER_ID,
  83  |     );
  84  |     expect(
  85  |       resultById.data.segmentId,
  86  |       "Segment ID must match created segment",
  87  |     ).toBe(String(segmentId));
  88  |     expect(
  89  |       resultById.data.segmentName,
  90  |       "Segment name must match created segment",
  91  |     ).toBe(segmentNameFromResp);
  92  | 
  93  |     // FIX: match real API response format
  94  |     /*if (resultById.data) {
  95  |       expect(resultById.data.segment_id).toBe(segmentId);
  96  |       expect(resultById.data.segment_name).toBe(segmentNameFromResp);
  97  |       expect(typeof resultById.data.in_segment).toBe("boolean");
  98  | */
  99  |     if (resultById.state === "results") {
  100 |       expect(
  101 |         resultById.data.riskScore,
  102 |         "Risk score must be present when in segment",
  103 |       ).toBeTruthy();
  104 |       expect(
  105 |         resultById.data.riskTier,
  106 |         "Risk tier must be present when in segment",
  107 |       ).toBeTruthy();
  108 |       expect(
  109 |         resultById.data.patternStatus,
  110 |         "Pattern status must be present when in segment",
> 111 |       ).toBeTruthy();
      |         ^ Error: Pattern status must be present when in segment
  112 |     }
  113 | 
  114 |     console.log(`✅ Search by ID — ${resultById.state}`);
  115 | 
  116 |     // =========================
  117 |     // 6. Search by name
  118 |     // =========================
  119 |     const resultByName = await screeningPage.searchBySegmentName(
  120 |       CUSTOMER_ID,
  121 |       segmentNameFromResp,
  122 |     );
  123 | 
  124 |     expect(["results", "not-in-segment"]).toContain(resultByName.state);
  125 | 
  126 |     expect(resultByName.data.segment_name).toBe(segmentNameFromResp);
  127 | 
  128 |     console.log(`✅ Search by name — ${resultByName.state}`);
  129 | 
  130 |     // =========================
  131 |     // 7. Invalid segment
  132 |     // =========================
  133 |     const errorResult = await screeningPage.searchBySegmentId(
  134 |       CUSTOMER_ID,
  135 |       "999999999",
  136 |     );
  137 | 
  138 |     // depends on backend behavior
  139 |     expect(errorResult.state).toBe("error");
  140 |     expect(errorResult.message).toBe("Segment not found");
  141 | 
  142 |     console.log("✅ Customer screening journey completed");
  143 |   });
  144 | });
  145 | 
```