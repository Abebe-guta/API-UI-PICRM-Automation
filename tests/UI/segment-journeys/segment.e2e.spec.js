...
test.describe("Full user journey: Segment → Pattern → Optimize", () => {
  ...
  test("Navigate from segment management to pattern to optimize", async ({
    sharedPage,
    builder,
    segmentAPI,
    createdSegment,
    ...
    */
    const { id: createdId, name: createdName } = createdSegment;

    await segmentPage.page.reload({
    ...
    expect(options.some((o) => o.includes(createdName))).toBeTruthy();

    // Select the newly created segment then delete it
    await segmentPage.selectSegment(createdName);
    ...
    expect(optionsAfter).not.toContain(createdName);
    ...
    const initiallyEnabled =
      ...
      !(await optimizePage.page
        .locator(optimizePage.locators.cutoff.saveButton)
